...
let allCaps = runners.map((run) => {
    return {"id": run.id, "first_name": run.first_name.toUpperCase(), "last_name": run.last_name, "email": run.email, "shirt_size": run.shirt_size, "company_name": run.company_name, "donation": run.donation}
});
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce((total, current) => total += current.donation, 0);
